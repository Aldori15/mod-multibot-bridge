...
`TALENT_APPLY_V1`, `TALENT_SPEC_APPLY_V1`, `CRAFT_RECIPE_TARGET_V1` and P3A `ITEM_DEPOSIT_EXACT_V1` are complete and runtime validated.
...
The bank / guild-bank comparison is closed for the current roadmap with P3A `ITEM_DEPOSIT_EXACT_V1`. P3B exact BANK withdrawal and P3C exact GBANK withdrawal are explicitly deferred because their current snapshots do not expose a selectable physical source stack end to end. The generic `SOURCE_STALE` UI text and equipped-bag reassignment remain low-priority residuals. The normal roadmap now resumes with `LOOT_RULE_EXACT_ITEM_ADD_REMOVE`; deferred work must not interrupt that sequence, including P3B/P3C, SELL_GREY/core-API follow-up and final Firestone/Spellstone TEMP_ENCHANT revalidation.
